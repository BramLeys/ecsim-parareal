...
int spatial_coarsening_parameter_test(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-f" && i + 1 < argc) {
            ...
            std::cerr << "Usage: " << argv[0] << " -f <fine timestep> -nx <number of gridpoints> -n <number of threads> -t <time interval [0,t]> -c <coarse timestep> -tr <parareal threshold>" << std::endl;
            return 1;
        }
    }
...
#pragma omp parallel private(nthreads, tid) num_threads(num_thr)
    ...
    for (int j = 0; j < refinements; j++) {
        int refinement_number = j+1;
        int fine_Nx = Nx* refinement_number;
        ...
        save("Parareal_speedup_refining_from_512.txt", speedup);
    }
    return 0;
}
