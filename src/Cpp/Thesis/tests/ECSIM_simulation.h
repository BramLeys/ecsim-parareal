...
int ECSIM_simulation(int argc, char* argv[]) {
    ...
    energies(0) = Eold.sum();
    for (int i = 0; i < NT; i++) {
        solver.Step(Yn.col(i), 0, T, Yn.col(i+1));
    ...
    PRINT("Serial energy difference between beginning and end", abs((solver.Energy(Yn) - Eold).sum()) / abs(Eold.sum()));
    PRINT("Serial took", serial_time, "ms");
    save("simulation_result.txt", Yn(Eigen::all,seq(0,NT,NT)));
    save("simulation_energy.txt", energies);
